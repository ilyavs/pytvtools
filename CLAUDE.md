...
This copies `src/pytvtools_core/`, `tests/`, `notebooks/`, `jobs/`, and `demos/` to the
standalone repo, commits, then push to GitHub. The Databricks workspace auto-syncs from the git folder.

**Demo HTML products.** The canonical copies of the Pages-hosted demo charts live in
`demos/` in THIS repo and are carried to the core repo by `sync_core.py` (same push command):

- `demos/absorption_ratio.html` — the interactive absorption-ratio page, served from
  `https://ilyavs.github.io/pytvtools-core/demos/absorption_ratio.html`. It is generated by
  `notebooks/absorption_ratio.py` in `view` mode (Databricks only — reads the UC tables
  `workspace.chartdata.ohlcv` / `absorption_ratio_runs`), which writes the HTML to the
  `chart_output/` volume. To publish a new version: run the notebook, download the volume HTML,
  overwrite `demos/absorption_ratio.html`, then run the push workflow above. Never hand-edit it
  in the core repo.
- `demos/demo_chart.html` — regenerable AAPL demo via `demos/demo.py` (live WebSocket, no UC).
  Also feeds this repo's own GitHub Pages site (`ilyavs.github.io/pytvtools/`) via
  `.github/workflows/pages.yml`, which copies it to `_deploy/index.html`.
